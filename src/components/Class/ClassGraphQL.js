import "./Class.css"
import React, {useState, useEffect} from "react";

function ClassGraphQL(props) {

    const [classInfo, setClassInfo] = useState({});
    const url = "https://api.peterportal.org/graphql"

    useEffect(() => {
        const fetchData = async () => {

            const query = ` 
                query {
                    course(id:"${props.name}"){
                        title
                        department_name
                        description
                        course_level
                    }
                }
            `
            const response = await fetch(url, {
                method: "POST",
                body: JSON.stringify({query}),
                headers: {
                    "Content-Type": "application/json"
                }
            });
            const data = await response.json();
            console.log(data);
            setClassInfo(data.data.course);
        }
        fetchData();
    }, [props.name])

    let info;
    if(classInfo){
        info = <div className="information">
        <h3 id="Department"> Department:  {classInfo.department_name} </h3>
        <h3 id="Description"> Description:  {classInfo.description} </h3>
        <h3 id="Course_level"> Course Level:  {classInfo.course_level} </h3>
        </div>
    }  else if (classInfo == null){
        info = <p>Class Not Found</p>
    }
    else{
        info = <p>Loading...</p>
    }
    return (
        <div className="classgl">
            <div>
                {info}
            </div>
        </div>
    )
}

export default ClassGraphQL;